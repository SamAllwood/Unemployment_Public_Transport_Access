## Interactive map (Leaflet) showing PTJA and PTJA-D across combined authorities in Northern England.
# 1. Setup ----------------------------------------------------------------
library(tidyverse)
library(knitr)
library(tidytransit)
...
interactive_datasets <- rbind(LCR_dataset_interactive, 
...
# Colour palettes based on combined datasets
...
jobs_pal <- colorQuantile(palette = "Spectral",                       
                         domain = interactive_datasets@data$Job_density,
                         n=5,
                         reverse = TRUE)
...
# Save Map
htmlwidgets::saveWidget(map, file="docs/index.html", selfcontained = FALSE)
